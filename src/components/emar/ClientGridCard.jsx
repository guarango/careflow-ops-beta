...
function getTodayDots(medications, logs) {
  ...
  return activeMeds.map(med => {
    ...
    if (todayLogs.length > 0) {
      ...
      if (last.status === "Administered") return "green";
      if (last.status === "Missed" || last.status === "Refused") return "red";
      return "amber";
    }
    return "amber";
  });
...
export default function ClientGridCard({ client, medications, logs, onClick, index }) {
  ...
  const dots = getTodayDots(medications, logs);
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-3 gap-1.5">
        ...
        <p className="text-xs text-muted-foreground">
          {activeMeds === 0 ? "No medications" : `${activeMeds} active medication${activeMeds !== 1 ? "s" : ""}`}
        </p>

        {/* Med pass dot indicators */}
        {dots.length > 0 && (
